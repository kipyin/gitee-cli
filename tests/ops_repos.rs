...
fn unstar_deletes_user_starred() {
    ...
    let mock = server
        .mock(
            "DELETE",
            api_path("/user/starred/oschina/gitee-cli").as_str(),
        )
        .with_status(204)
        .create();
    client(&server)
...
fn watch_and_unwatch_hit_subscriptions() {
    ...
    let put = server
        .mock(
            "PUT",
            api_path("/user/subscriptions/oschina/gitee-cli").as_str(),
        )
        .with_status(204)
        .create();
    client(&server)
