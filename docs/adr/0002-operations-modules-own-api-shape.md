...
- **Milestone collection** — list, get, create, and edit use `/repos/{owner}/{repo}/milestones…` in `src/api/milestones.rs`. The collection read is `Milestones::list` (paged).

## Decision

**All API shape knowledge lives in operations modules** — `src/api/pulls.rs`, `src/api/issues.rs`, `src/api/releases.rs`, `src/api/milestones.rs`, `src/api/repos.rs` — behind typed methods (`Pulls`, `Issues`, `Releases`, `Milestones`, `Repos`).
...
| Operations | `src/api/{pulls,issues,releases,milestones,repos}.rs` | Paths, query keys, form/JSON bodies, Gitee quirks |
...
- Duplicating path templates or form keys outside `src/api/{pulls,issues,releases,milestones,repos}.rs` is a review failure.
